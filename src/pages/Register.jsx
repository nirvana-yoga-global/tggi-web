...
function Field({ label, hint, error, required, children }) {
  return (
    <div className="mb-7">
      ...
      {hint && (
        <p className="font-sans text-forest/55 text-base mb-2 leading-snug">{hint}</p>
      )}
      {children}
      {error && (
...
const inputVerifiedClass =
  'w-full min-h-[52px] px-4 py-3 rounded-xl border-2 border-forest bg-forest/5 ' +
  'font-sans text-forest text-lg leading-snug focus:outline-none'
...
/* ── Success screen with leaf draw animation ── */
    ...
    <div className="flex flex-col items-center text-center py-10 px-4">
      ...
      {nygId && (
        <p className="font-sans text-forest/60 text-base mb-4">
          Your NYG ID: <strong className="text-forest font-semibold font-mono">{nygId}</strong>
        </p>
      )}
...
export default function Register() {
  ...
  function setField(field, value) {
    ...
    if (field === 'email') {
      setOtp({ sent: false, value: '', verified: false, loading: false, error: '' })
    }
  }
  ...
  async function handleSendOtp() {
    ...
    setErrors(e => ({ ...e, email: '' }))
    setOtp(o => ({ ...o, loading: true, error: '' }))
    try {
      const data = await callEdgeFunction('send-otp', { email })
      if (data.success) {
        setOtp(o => ({ ...o, sent: true, loading: false }))
      } else {
        setOtp(o => ({ ...o, loading: false, error: data.error || 'Failed to send OTP. Please try again.' }))
      }
    } catch {
      setOtp(o => ({ ...o, loading: false, error: 'Network error. Please try again.' }))
    }
  }

  async function handleVerifyOtp() {
    if (!otp.value.trim()) {
      setOtp(o => ({ ...o, error: 'Please enter the code from your email.' }))
      return
    }
    setOtp(o => ({ ...o, loading: true, error: '' }))
    try {
      const data = await callEdgeFunction('verify-otp', { email: form.email.trim(), otp: otp.value.trim() })
      if (data.verified) {
        setOtp(o => ({ ...o, verified: true, loading: false, error: '' }))
      } else {
        setOtp(o => ({ ...o, loading: false, error: data.error || 'Invalid or expired code. Please try again.' }))
      }
    } catch {
      setOtp(o => ({ ...o, loading: false, error: 'Network error. Please try again.' }))
    }
  ...
  function validate() {
    ...
    else if (!otp.verified)       e.email       = 'Please verify your email address before submitting.'
  ...
  async function handleSubmit(evt) {
    ...
    if (!supabase) {
      setSubmitError('Supabase is not configured. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to .env and restart.')
      return
    }
    ...
    try {
      ...
      if (dupEmail) {
        setErrors(e => ({ ...e, email: 'This email is already registered. Please use a different email.' }))
        setLoading(false)
        return
      }
      ...
      /* 3. Generate NYG ID */
      const generatedNygId = await generateNygId()
    ...
    return (
      <div className="min-h-screen bg-cream pt-20">
        <div className="max-w-xl mx-auto px-6 py-16 sm:py-20">
          <SuccessScreen fullName={form.full_name.trim()} plantName={form.plant_name.trim()} nygId={nygId} />
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-xl mx-auto px-6 py-12 sm:py-16">
        ...
        <div className="mb-10 sm:mb-12">
          ...
          <p className="font-sans text-forest/70 text-lg leading-relaxed">
            Join the movement — plant something, photograph it, and share your
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <Field
            label="Phone Number"
            hint="Kept private — used to link your account."
            required
            error={errors.phone}
          >
            <input
          ...
          {/* 3. Email with OTP verification */}
          <Field
            label="Email Address"
            hint="Required — we'll send a verification code."
            required
            error={errors.email}
          >
            <div className="flex gap-2 items-stretch">
              <input
                ...
                className={`flex-1 ${otp.verified ? inputVerifiedClass : errors.email ? inputErrorClass : inputClass}`}
                autoComplete="email"
              ...
              {!otp.verified && (
                <button
                  ...
                  disabled={otp.loading || !form.email.trim()}
                  className="
                    flex-shrink-0 px-4 min-h-[52px] rounded-xl
                    border-2 border-forest text-forest
                  ...
                  "
                >
                  {otp.loading && !otp.sent ? <Spinner /> : otp.sent ? 'Resend' : 'Send OTP'}
                </button>
              )}
            </div>
            ...
            {otp.verified && (
              <div className="flex items-center gap-2 mt-2.5">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <circle cx="12" cy="12" r="10" fill="#1f3d2b" />
                  <path d="M7 12.5 L10.5 16 L17 9" stroke="#faf7f0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                <span className="font-sans text-forest font-semibold text-base">Email verified</span>
              </div>
            )}

            {/* OTP entry — shown after Send OTP, before verification */}
            {otp.sent && !otp.verified && (
              <div className="mt-3">
                <p className="font-sans text-forest/60 text-sm mb-2">
                  A 6-digit code was sent to <strong>{form.email}</strong>. Expires in 10 minutes.
                </p>
                <div className="flex gap-2 items-stretch">
                  <input
                    type="text"
                    ...
                    onChange={e => {
                      ...
                      setOtp(o => ({ ...o, value: v, error: '' }))
                    }}
                    placeholder="000000"
                    className={`
                      flex-1 min-h-[52px] px-4 py-3 rounded-xl border-2
                      font-sans text-forest text-xl tracking-[0.3em] text-center leading-snug
                      focus:outline-none transition-colors duration-150 bg-white
                      ${otp.error ? 'border-terracotta' : 'border-forest/20 focus:border-forest'}
                    `}
                  />
                  <button
                    type="button"
                    ...
                    className="
                      flex-shrink-0 px-5 min-h-[52px] rounded-xl
          ...
          {/* Rest of form — unlocked after email verified */}
          {otp.verified && (
            <>
              {/* 4. Plant Nickname */}
